...
INT_PTR CALLBACK Matrix(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
        if (LOWORD(wParam) == IDOK)
        {
            ...
            {
                MessageBox(hDlg, L"Неправильний формат вводу даних", L"Помилка", MB_OK | MB_ICONERROR);
                break;
            }

            if (min > max || N < 1 || N > 10 || abs(min) > 99999 || abs(max) > 99999)
            {
                MessageBox(hDlg, L"Некоректно введені дані", L"Помилка", MB_OK | MB_ICONERROR);
                break;
            }
            ...
            MatrixDialog::SendCopyData(hWnd2, GetParent(hDlg), data, sizeof(data));
...
void MatrixDialog::SendCopyData(HWND hWndDest, HWND hWndSrc, void* lp, long cb)
{
    COPYDATASTRUCT cds;
...
void MatrixDialog::EndWnd()
{
    HWND hWnd2 = FindWindow(L"OBJECT2", NULL);
    HWND hWnd3 = FindWindow(L"OBJECT3", NULL);

    if (hWnd2) PostMessage(hWnd2, WM_DESTROY, NULL, NULL); //записує в чкпгу повідомлень
    if (hWnd3) PostMessage(hWnd3, WM_DESTROY, NULL, NULL);
}

void MatrixDialog::StartWnd(HINSTANCE hInst, HWND hWnd)
{
