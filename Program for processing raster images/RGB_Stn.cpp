...
static INT_PTR CALLBACK RGB_MOD(HWND hDlg, UINT iMessage, WPARAM wParam, LPARAM lParam);
...
int WindowRGB(HINSTANCE hInst, HWND hWnd, int* r, int* g, int* b)
{
    int res = DialogBox(hInst, MAKEINTRESOURCE(IDD_WORK_MOD3), hWnd, RGB_MOD);
...
INT_PTR CALLBACK RGB_MOD(HWND hDlg, UINT iMessage, WPARAM wParam, LPARAM lParam)
{
    switch (iMessage)
    {
    case WM_INITDIALOG:
...
void OnRedButton(HWND hDlg) {
    ...
    pressedRBut = 1;
    SetDlgItemText(hDlg, ID3_TEXT3, "Red:");
    PrintCurPos(hDlg);
}
...
void OnGreenButton(HWND hDlg) {
    ...
    pressedGBut = 1;
    SetDlgItemText(hDlg, ID3_TEXT3, "Green:");
    PrintCurPos(hDlg);
}
...
void OnBlueButton(HWND hDlg) {
    ...
    pressedBBut = 1;
    SetDlgItemText(hDlg, ID3_TEXT3, "Blue:");
    PrintCurPos(hDlg);
}
