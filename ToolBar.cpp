...
void ToolBar::OnCreate(HWND hWnd, HINSTANCE hInst)
{
    ...
    tbb[0].idCommand = ID_TOOL_DOT; //цей ID буде у повідомленні WM_COMMAND
    ...
    tbb[4].idCommand = ID_TOOL_RGB;

    tbb[5].iBitmap = 5;
    tbb[5].fsState = TBSTATE_ENABLED;
    tbb[5].fsStyle = TBSTYLE_BUTTON;
    tbb[5].idCommand = ID_TOOL_CONTRAST;

    tbb[6].iBitmap = 6;
    tbb[6].fsState = TBSTATE_ENABLED;
    tbb[6].fsStyle = TBSTYLE_BUTTON;
    tbb[6].idCommand = ID_TOOL_SHADOW;

    tbb[7].iBitmap = 7;
    tbb[7].fsState = TBSTATE_ENABLED;
    tbb[7].fsStyle = TBSTYLE_BUTTON;
    tbb[7].idCommand = ID_TOOL_LIGHT;
    ...
    hwndToolBar = CreateToolbarEx(hWnd, //батьківське вікно
        ...
        8, //кількість зображень у BITMAP
        hInst,
        IDB_BITMAP1, //ID ресурсу BITMAP
        tbb,
        8, //кількість кнопок (разом з роздільником)
